...
应用管理模块，按 `app.xxx(...)` 调用。常用方法包括 `currentPackage`、`selfPackage`、`launch`、`forceStop`、`getList`、`getName`、`getVersion`、`getIcon`、`isInstalled`、`install`、`uninstall`、`clear`、`openUrl`。`getList(false)` 返回应用信息数组表。
...
网络请求模块，按 `https.xxx(...)` 调用。`get(url, timeout)`、`post(url, data, headers, timeout)`、`postMultipart(url, fileName, fileData, timeout)` 返回 `{code=..., data=...}` 或 `{code=..., body=...}` 结构，字节结果在 Lua 中按数组表/字节桥接值处理。
...
飞浆 OCR 模块，按 `ppocr.xxx(...)` 调用。iOS 版本不带 Android `displayId` 参数，识别结果统一为 Lua table。
